...
- For DB changes: flag migration strategy, Django model impact, reversibility
- For API changes: flag breaking vs. non-breaking per AGENTS.md versioning strategy
- For enrichment providers: flag dependency graph impact, output_keys, Provider ABC contract
...
- Cover relevant dimensions: architecture, data model, migration safety, error handling, testing strategy, API contract
...
When a user makes a feature request without explicit design context (e.g., "add X to the API"), suggest brainstorming before diving in:
