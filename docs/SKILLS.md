...
|---|---|
| `reviewing-code-claude` | Django/FastAPI dimensions; ruff lint check; TDD discipline; pipeline stage contracts; migration safety; JSON logging; Iron Law + rationalization-prevention table; Phase 3.5 verification gate |
| `shipping-work-claude` | `uv run pytest --no-cov` + `uv run ruff check` in pre-ship.sh; `#<n> [type]: <desc>` commit convention; systemd restart step; Iron Law + HARD-GATE on partial issue closure |
| `brainstorming` | Project conventions (docs/plans/ path, commit format); invokes using-git-worktrees after design approval; Django/FastAPI stack context; proactive-suggestion mode |
...
Skill frontmatter must include `triggers` in `metadata`. New project-specific skills go in `skills/<name>/` with a `.claude/skills/<name>` symlink to `../../skills/<name>`. Cross-project skills belong in `gregoryfoster/skills`.
