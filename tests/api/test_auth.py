...
class TestAPIKeyAuth:
    ...
    async def test_revoked_key_returns_403(self, client, tmp_db, revoked_api_key):
        ...
        response = await client.get("/v1/health", headers={"X-API-Key": revoked_api_key.raw_key})
    ...
    async def test_expired_key_returns_403(self, client, tmp_db, expired_api_key):
        ...
        response = await client.get("/v1/health", headers={"X-API-Key": expired_api_key.raw_key})
        assert response.status_code == 403
    ...
    async def test_valid_key_passes(self, client, tmp_db, valid_api_key):
        ...
        response = await client.get("/v1/health", headers={"X-API-Key": valid_api_key.raw_key})
    ...
    async def test_valid_key_updates_last_used_at(self, client, tmp_db, valid_api_key):
        ...
        await client.get("/v1/health", headers={"X-API-Key": valid_api_key.raw_key})

        conn = sqlite3.connect(str(tmp_db))
        row = conn.execute(
            "SELECT last_used_at FROM keys_apikey WHERE id = ?",
            (valid_api_key.key_id,),
        ).fetchone()
        conn.close()
